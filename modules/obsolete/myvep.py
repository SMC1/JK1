...
def ircr_normal():
	normDB = {}
	inFile = open('/data1/Sequence/ucsc_hg19/annot/ircr_blood.dat')
	for line in inFile:
		...
		alt = colL[4]
		cnt = int(colL[5])
		normDB[(chr,sta,end,ref,alt)] = cnt
	return(normDB)

def process_vep_vcf(inFileN, sampN, outFileN):
#	normDB = ircr_normal()
#	vepH = parse_vep_vcf(inFileN)
#	inFile = open(inFileN)
#	outFile = open(outFileN, 'w')
	pass


def process_cancerscan_vep(inFileN, sampN, outFileN):
	normDB = ircr_normal()
	...
	for line in inFile:
		...
		datL = colL[9].split(':')
		AD = datL[formatL.index('AD')]
		if AD == '0,0':## fix!!! to separate normal from tumor (normal sometimes come after tumor)
			datL = colL[10].split(':')
			AD = datL[formatL.index('AD')]

		if (chr, pos, ref, alt) in vepH:
			chr_tmp = chr
			infoH = print_vep_item(vepH[(chr,pos,ref,alt)])
			ircr = 0
			n_ref = int(AD.split(',')[0])
			n_alt = int(AD.split(',')[1])
			if ('chr%s' % chr,int(pos),int(pos),ref,alt) in normDB:
				ircr = normDB[('chr%s' % chr,int(pos),int(pos),ref,alt)]
			for gene in infoH:
				outFile.write('%s\tchr%s\t%s\t%s\t%s\t%s' % (sampN, chr_tmp, pos, pos, ref, alt))
				outFile.write('\t%s\t%s' % (n_ref, n_alt))
				if gene == '-':
					out_gene = ''
				else:
					out_gene = gene
				...
				if gene in cs_gene:
					cs_tag = ';CS'
				outFile.write('\tIRCR:%s%s\t%s\t%s\t%s\t%s\t%s\n' % (ircr,cs_tag, infoH[gene]['strand'], out_gene, infoH[gene]['ch_dna'], infoH[gene]['ch_aa'], infoH[gene]['ch_type']))
		##if
	##line
	outFile.flush()
	outFile.close()
...
#	print_vep(temp)
#	process_mutect_vep('/EQL1/pipeline/CS20140618_xsq2mut/IRCR_LC14_335_T_CS/IRCR_LC14_335_T_CS_mutect_vep.vcf','/EQL1/pipeline/CS20140618_xsq2mut/IRCR_LC14_335_T_CS/IRCR_LC14_335_T_CS_mutect_vep.dat')
#	process_cancerscan_vep('/home/ihlee/indel_test/IRCR_GBM14_416_T_CS.indels_vep.vcf', '/home/ihlee/indel_test/IRCR_GBM14_416_T_CS.indels_vep.dat', postfix='.indels_vep.vcf')
#	for PREFIX in ['IRCR_GBM14_482','IRCR_LC14_278','IRCR_GBM14_485','IRCR_GBM14_488','IRCR_MBT14_166','IRCR_MBT14_167']:
#		inFN='/home/ihlee/indel_test/%s.indels_vep.vcf' % PREFIX
#		outN='/home/ihlee/indel_test/%s.indels_vep.dat' % PREFIX
#		process_cancerscan_vep(inFN, outN, postfix='.indels_vep.vcf')
	process_cancerscan_vep('/EQL5/pipeline/CS_mut/IRCR_GBM14_459_T01_CS/IRCR_GBM14_459_T01_CS.indels_filter_vep.vcf','IRCR_GBM14_459_T01_CS','/EQL5/pipeline/CS_mut/IRCR_GBM14_459_T01_CS/IRCR_GBM14_459_T01_CS.indels_filter_vep.dat')
