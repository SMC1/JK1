getChrLen<-function(chr, chromsizeFile='chromsizes_hg19.txt')
{
  chrLenDF = read.table(chromsizeFile,header=F)
  return(chrLenDF[chrLenDF[,1]==sprintf('chr%s',chr),2])
}

drawTraj<-function(fileN, sampN, chr, lColCommon=NaN, cnaMaxAbs=3, chromsizeFile='chromsizes_hg19.txt') {
  chrLen = getChrLen(chr, chromsizeFile)

  df = read.table(fileN, header=F)
  colnames(df) <- c('chrom','loc.start','loc.end','value')
  ...
  df_ft = df[df$chrom == sprintf('chr%s', chr), c(2,3,4)]
