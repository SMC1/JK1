...
#			continue
		...
		if pbs:
			os.system('echo "python ~/JK1/NGS/pipeline/pipe_s_rsq2expr.py -i %s -n %s -p %s -c %s -s %s -g %s" | qsub -N %s -o %s/%s.Rsq_expr.qlog -j oe' % \
			(inputFileP2, sampN, projectN, False, server, genome, sampN, storageBase+projectN+'/'+sampN, sampN))	

		else:
			os.system('(python ~/JK1/NGS/pipeline/pipe_s_rsq2expr.py -i %s -n %s -p %s -c %s -s %s -g %s) 2> %s/%s.Rsq_expr.qlog' % \
