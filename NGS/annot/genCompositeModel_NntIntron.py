...
def genCompositeModel(outTextFileName,outFaFileName,intronSize=100): 
	...
	#for chrNum in [1]:
		...
		for strand in ['+','-']:
			...
			txnLocusL = [mygenome.locus('%s:%s-%s%s' % (chrom,h['txnSta'],h['txnEnd'],strand),h['geneId']) for h in filter(lambda x: x['strand']==strand, geneH[chrom])]
		...
		for txnLoc in txnLocusL_combined:
			...
			for h in filter(lambda x: x['geneId'] in txnLoc.id, geneH[chrom]):
...
genCompositeModel('/data1/Sequence/compositeModel_hg19/kgLinc_100Int.txt','/data1/Sequence/compositeModel_hg19/kgLinc_100Int.fa',100)
