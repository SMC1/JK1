	...
	#for chrNum in [1]:
		...
		for txnLoc in txnLocusL_combined:
			...
			for i in range(len(exnLocusL)):
				...
				if i > 0:
					exnLocCopy.chrSta -= min(50, int((exnLocusL[i].chrSta - exnLocusL[i-1].chrEnd)/2))

				if i < len(exnLocusL)-1:
					exnLocCopy.chrEnd += min(50, int((exnLocusL[i+1].chrSta - exnLocusL[i].chrEnd)/2))
...
genKgCompositeModel('/data1/Sequence/compositeModel_hg19/kg_100ntIntron/knownGeneAnnotationTranscriptCompositeModel.txt','/data1/Sequence/compositeModel_hg19/kg_100ntIntron/knownGeneAnnotationTranscriptCompositeModel.fa')
