...
if not ('-i' in optH):
	...
	print 'Usage: sam2bed_batch.py -i [input file dir]'
	sys.exit(0)
...
#		continue
	...
	if sampN in ['G17224.TCGA-06-0139-01A-01R-1849-01.2_30nt']:

		if '-p' in optH:

			os.system('echo "samtools view -S -b %s/%s.sorted.sam | bamToBed -i stdin | grep -e \'^chr1[^0-9]\' > %s/%s_chr1.bed" \
			...
			os.system('(samtools view -S -b %s/%s.sorted.sam | bamToBed -i stdin | grep -e \'^chr1[^0-9]\' > %s/%s_chr1.bed) 2> %s/%s.qlog' % \
