...
def fusion_proc_annot(inReportFileName,outReportFileName):
	...
	for line in open(inReportFileName):
		...
		for (bp,teStr) in ((bp1,teStr1),(bp2,teStr2)):
			...
			geneStatL.append((bp1.split(':')[0],bp,teStr,teL,geneS,bp_geneS,geneInfo,censusInfo,goInfoS,keggInfoS,biocartaInfoS))


		(chrom1,bp1,teStr1,teL1,geneS1,bp_geneS1,geneInfo1,censusInfo1,goInfoS1,keggInfoS1,biocartaInfoS1) = geneStatL[0]
		(chrom2,bp2,teStr2,teL2,geneS2,bp_geneS2,geneInfo2,censusInfo2,goInfoS2,keggInfoS2,biocartaInfoS2) = geneStatL[1]
		...
		outReportFile.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % \
			(sampN, splice_type, type, bp1,bp2, teStr1,teStr2, ','.join(frameL), \
