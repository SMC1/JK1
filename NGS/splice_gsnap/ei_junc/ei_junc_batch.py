...
#			continue
		...
		os.system('echo "~jinkuk/JK1/NGS/splice_gsnap/ei_junc/ei_junc.py -i %s/%s_splice.gsnap -o %s/%s_ei_EGFR.dat -s %s" \
			| qsub -N %s -o %s/%s.ei_EGFR.qlog -j oe' % (inDirName,sampN, outDirName,sampN, sampN, sampN, outDirName,sampN))
