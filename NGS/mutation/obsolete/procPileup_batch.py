...
def main(inDirName,outDirName,fileNamePattern,pbs):
	...
	for sampN in sampNameL[1:]:
		...
		if pbs:
			os.system('echo "~/JK1/NGS/mutation/procPileup.py < %s/%s.pileup > %s/%s.pileup_proc" | qsub -N %s -o %s/%s.pileup_proc.log -j oe' % \
				(inDirName,sampN, outDirName,sampN, sampN, outDirName,sampN))
		else:
			os.system('(~/JK1/NGS/mutation/procPileup.py < %s/%s.pileup > %s/%s.pileup_proc) 2> %s/%s.pileup_proc.log' % (inDirName,sampN, outDirName,sampN, outDirName,sampN))
...
main('/EQL1/NSL/exome_bam/mutation/link', '/EQL1/NSL/exome_bam/mutation/link', '(.*)\.pileup', True)
