...
//        return RECT_HEIGHT * no_genes;
    ...
    var redraw = function(samples_visualized, track, hugo) {
        ...
        var mut = sample_enter.append('rect')
            .attr('class', 'mut')
            .attr('fill', function(d) {
				if ((hugo.indexOf('RPKM') != -1) || (hugo.indexOf('CNA') != -1) || (hugo.indexOf('EXPR') != -1)) {
    ...
    var makeQtip = function() {
        var formatMutation = function(sample, hugo) {
			...
			var freq_sub = '';
			var fraction = '';
			if ((hugo.indexOf("METH") != -1) || (hugo.indexOf('RPKM') != -1) || (hugo.indexOf("CNA") != -1) || (hugo.indexOf('EXPR') != -1) || (hugo.indexOf('PATH') != -1) || (hugo.indexOf('TYPE') != -1)) {
				if(mutation != null)
				...
				if ((freq !== 'nofreq') && (freq !== 'null')) {
					...
					freq_sub = '<b>Freq: </b>' + String(freq).substr(0,freq.indexOf('.')+3) + ' ' + fraction + '<br/>';
				}
...
//                    console.log(ui.value);
                    ...
                    if(ui.value == 2.25) {
                        ...
                        var allsample = alltrack.selectAll('.sample').insert('text')
                            ...
                            .attr('font-size', 10)
                            .text( function(d) {
								if ((d.hugo.indexOf('METH') != -1) || (d.hugo.indexOf("RPKM") != -1) || (d.hugo.indexOf("CNA") != -1) || (d.hugo.indexOf('EXPR') != -1) || (d.hugo.indexOf('PATH') != -1) || (d.hugo.indexOf('TYPE') != -1)) {
									var value_d = query.data(d.sample, d.hugo, 'mutation');
									if (value_d != null) {
										var sub_value = String(value_d).substr(0, value_d.indexOf('.')+3);
										return sub_value;
									}
								}
								else{
                                	...
                                	if((freq != null) && (freq != 0)) {
                                		var sub_freq = String(freq).substr(0, freq.indexOf('.')+3);
                                    	return sub_freq;
									}
                                }return null;
                            });
