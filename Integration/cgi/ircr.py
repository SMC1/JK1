...
#		from splice_fusion where (find_in_set("%s",gene_sym1) or find_in_set("%s",gene_sym2)) group by samp_id, locate(":Y",frame)>1' % (geneN,geneN))
    ...
    for (sId,) in results:
        ...
        print '<tr>',
        print '<td nowrap><a href="/cgi-bin/yn_test/ircr_yn_id.py?%s" target=_blank>%s</td>' % (sId, sId),
        ...
        for row in conditionL:
            ...
            if len(results2) == 1 and results2[0][0] not in (0,'0'):
                ...
                if row_wt:
                    ...
                    if row[1] == 't_fusion':
                        if row[4] == 'in':
                            print '<td><a href=/cgi-bin/yn_test/ircr_yn_fusion_detail.py?dbN=%s&geneN=%s&sId=%s&flag=in> %s </td>' % (dbN, geneN, sId, value),
                        else :
                            print '<td><a href=/cgi-bin/yn_test/ircr_yn_fusion_detail.py?dbN=%s&geneN=%s&sId=%s&flag=off> %s </td>' % (dbN, geneN, sId, value)
                    else :
                        print '<td>%s</td>' % value
