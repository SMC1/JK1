...
class Crawler:
	...
	def crawlingData_skip(self, geneN, condition_skip_list, cnd_cnt):
		cursor = self.connect_db()
		data = []
		cnd = "%" + condition_skip_list[cnd_cnt] + "%"
		cursor.execute('select samp_id, delExons, nReads from splice_skip where gene_sym = "%s" and delExons like "%s" ' % (geneN, cnd))
		data = cursor.fetchall()
	...
	#2. add data to the sample_list dic
	def crawlingData_del(self, geneN, del_list, cnt):
		cursor = self.connect_db()
		data = []
		cursor.execute('select samp_id, juncAlias, nReads from splice_eiJunc where gene_sym="%s" and nReads>10 and juncAlias = "%s" group by samp_id' % (geneN, del_list[cnt]))
		data = cursor.fetchall()

		for (samp_id, juncAlias, nReads) in data:
			self.sample_list[samp_id]["DEL " + del_list[cnt]] = nReads
