...
class Parser:
	...
	def parsing_input(self, items, geneN, dbN):
		item_tmp_list = items.split(',')
	
		for item in item_tmp_list:
			...
			if item in self.tag_index:
				...
				self.item_list.append(item)
				self.deletion_item_list.append(item_substr)
	...
	#getter - del_list of the input query
	def get_deletion_item_list(self):
		return self.deletion_item_list
