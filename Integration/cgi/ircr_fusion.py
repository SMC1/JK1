...
def Process(dbN, geneN, sId, flag):
    ...
    if flag == 'off':
        cursor.execute("select samp_Id, nPos, frame, loc1, loc2, gene_sym1, gene_sym2, ftype, exon1, exon2 from splice_fusion where (find_in_set('%s',gene_sym1) or find_in_set('%s',gene_sym2)) and locate(':Y',frame)=0 and samp_id='%s' order by nPos desc" % (geneN,geneN,sId))
    else :
        cursor.execute("select samp_Id, nPos, frame, loc1, loc2, gene_sym1, gene_sym2, ftype, exon1, exon2 from splice_fusion where (find_in_set('%s',gene_sym1) or find_in_set('%s',gene_sym2)) and locate(':Y',frame)!=0 and samp_id='%s' order by nPos desc" % (geneN,geneN,sId))
