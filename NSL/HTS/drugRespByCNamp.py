...
#	cursor.execute('SELECT samp_id FROM splice_skip_AF \
#		where gene_sym ="EGFR" and delExons like "%s%s%s" and nPos>2 and frame like "%s:Y%s"' % ('%','2-7','%','%','%'))
#	v3IdL = [x for (x,) in cursor.fetchall()]
	...
	for gN in geneL:
		...
		cursor.execute('SELECT samp_id,gene_sym,value_log2 FROM xsq_cn \
			where gene_sym ="%s"' % gN)
...
#					continue
		...
		tempFile.close()
		os.system('Rscript ~/JK1/NSL/HTS/drugRespByAlt.R %s %s %s %s %s' % (tempFileName, outFileName,plot,outPlotDirName,plottype))

optL, argL = getopt.getopt(sys.argv[1:],'i:o:e:',[])

optH = mybasic.parseParam(optL)

#main('/home/heejin/DrugScreening/Input/150409_AUC_v4.csv','/home/heejin/DrugScreening/','/home/heejin/DrugScreening/result_AUC_QC_amp_woVIII_v4.txt',geneL=['EGFR'],cutoff=0.5, plottype='AUC',plot='TRUE', outPlotDirName = '/home/heejin/DrugScreening/figure/CNAv4')
main('/home/heejin/DrugScreening/Input/150409_AUC_v4.csv','/home/heejin/DrugScreening/','/home/heejin/DrugScreening/result_AUC_QC_amp_v4.txt',geneL='cs',cutoff=0.5, plottype='AUC',plot='TRUE', outPlotDirName = '/home/heejin/DrugScreening/figure/CNAv4')
