test <- function(
  ...
  for (drug in drugNameL) {
    ...
    if (TRUE %in% !is.na(df_ft$Alt)){
      ...
      mis_type_all = alt_type_all[grep("Missense",alt_type_all,ignore.case=T)]
      
      df_ft$type = NA
      df_ft$type[grep("inframe",df_ft$Alt,ignore.case=T)] = "Inframe"
      df_ft$type[grep("missense",df_ft$Alt,ignore.case=T)] = "Missense"
      df_ft$type[!is.na(df_ft$Alt) & is.na(df_ft$type)] = "Truncating"
      
      mis_type = unique(gsub("p.","",regmatches(mis_type_all,regexpr("p\\.[A-Z]{1}.*[A-Z]{1}",mis_type_all,perl=T))))
      
      for (ms in mis_type){
        df_ft$type[grep(gsub(",",",p.",ms),df_ft$Alt)] = ms
      }
      
      alt_type = setdiff(df_ft$type,NA)
      
      for (alt in alt_type){
      
      df_ft$group = 'WT'
      df_ft$group[grep(alt,df_ft$type)]='Alt'
