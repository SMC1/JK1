...
def main(inDrugFileName,outDirName,outFileName,geneL='wg',cutoff=0.05, plottype='AUC', plot='FALSE', outPlotDirName='/home/heejin/DrugScreening/figure',seqType='WTS'):
	...
	geneL = geneL + ['BTK','BLK','BMX']

	# gene expression
	
	outFile = open(outFileName, 'w')
	outFile.write('Drug\tGene\tp_pearson\tr_pearson\thighN\tlowN\twilcox_p\tttest_p\n')
	outFile.close()
	...
	for gN in geneL:
		...
		cursor.execute('SELECT samp_id,gene_sym,lg2_rpkm FROM rpkm_gene_expr_lg2 \
			where gene_sym ="%s"' % gN)
...
#				continue
		...
		tempFile.close()
		os.system('Rscript ~/JK1/NSL/HTS/drugRespByExpr.R %s %s %s %s %s' % (tempFileName, outFileName,plot,outPlotDirName,plottype))

optL, argL = getopt.getopt(sys.argv[1:],'i:o:e:',[])

optH = mybasic.parseParam(optL)

#main('/home/heejin/DrugScreening/Input/logIC50.csv','/home/heejin/DrugScreening/','/home/heejin/DrugScreening/result_logIC50_QC_expr_ibr.txt',geneL=['BTK','BLK','BMX'],cutoff=0.05, plottype='logIC50', plot='TRUE', outPlotDirName = '/home/heejin/DrugScreening/figure/Expr')
main('/home/heejin/DrugScreening/Input/150409_AUC_v4.csv','/home/heejin/DrugScreening/','/home/heejin/DrugScreening/result_AUC_QC_expr_v4.txt',geneL='cs',cutoff=0.05, plottype='AUC', plot='TRUE', outPlotDirName = '/home/heejin/DrugScreening/figure/EXPRv4')
