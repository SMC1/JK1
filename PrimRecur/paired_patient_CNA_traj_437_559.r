...
{
  if (type == 'prb') {
  df = read.table(sprintf('%s/%s_prb.dat',inPrbDirName,sId),header=T)
  } else {
...
)
{
  ...
  df = read.table(sprintf('%s/paired_df_CNA_prb_437.txt',inDirName),header=TRUE)
  ...
  for (i in 1:nrow(sIdPairDF)) {
    ...
    if (graphicsFormat == 'png') {
      png(sprintf("%s/CNA_traj/paired_CNA_traj_prb_%s_%s.png", inDirName,sId_p,sId_r))
    } else if (graphicsFormat== 'pdf') {
      pdf(sprintf("%s/CNA_traj/paired_CNA_traj_prb_%s_%s.pdf", inDirName,sId_p,sId_r))
    }
    ...
    drawTraj("prb",sId_p,'black'); par(new=T); drawTraj("seg",sId_p,'red'); text(totChrLen*0.02,cnaMaxAbs-cnaMaxAbs*2*0.03,sprintf('%s',sId_p),adj=c(0,1),cex=1.1)
    drawTraj("prb",sId_r,'black'); par(new=T); drawTraj("seg",sId_r,'red'); text(totChrLen*0.02,cnaMaxAbs-cnaMaxAbs*2*0.03,sprintf('%s',sId_r),adj=c(0,1),cex=1.1)
