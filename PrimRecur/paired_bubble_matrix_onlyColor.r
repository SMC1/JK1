  ...
  )
  {
    ...
    for (geneN in c('EGFR','CDKN2A','CDK4','CDKN2B','PDGFRA','PTEN','MDM2','CDKN2C','MDM4','RB1','MET','QKI','CDK6','NF1')){
      ...
      for (dType in c('CNA','RPKM')){
        ...
        par(new=T)
        rect(xSta,xSta,xEnd,xEnd,border='black',col=bluered(101)[dmean_scale],xaxt='n',yaxt='n', xaxs='i',yaxs='i',axes=F, ann=F,)
        #par(new=T)
        ...
        #plot(x=df_ft$val_p, y=df_ft$val_r, pch=21, cex=2, xlim=c(xSta,xEnd), ylim=c(xSta,xEnd), xaxs='i',yaxs='i', xaxt='n',yaxt='n',axes=F, ann=F,)
        #par(new=T)
        #plot(c(xSta,xEnd),c(xSta,xEnd), type='l',pch=22,lty=2, xlim=c(xSta,xEnd), ylim=c(xSta,xEnd), axes=F, xaxt='n', yaxt='n', ann=F, xaxs='i',yaxs='i')
