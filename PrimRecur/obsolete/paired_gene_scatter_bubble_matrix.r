...
)
{
  ...
  if (graphicsFormat == 'png') {
    png(sprintf("%s/scatter/paired_bubble_matrix.png", inDirName,geneN))
  } else if (graphicsFormat== 'pdf') {
    pdf(sprintf("%s/scatter/paired_bubble_matrix.pdf", inDirName,geneN))
  }
...
#     plot(df_ft$val_p, df_ft$val_r, xlim=c(xSta,xEnd), ylim=c(xSta,xEnd), xlab=sprintf('Prim, %s',lab), ylab=sprintf('Recur, %s',lab),cex.lab=0.9,cex.axis=0.9)
    ...
    symbols(x=df_ft$val_p, y=df_ft$val_r, circles=abs(df_ft$delta), inches=1/3, bg=df_ft$color, fg=NULL, xlim=c(xSta,xEnd), ylim=c(xSta,xEnd), xlab=sprintf('Prim, %s',lab), ylab=sprintf('Recur, %s',lab))
    par(new=T)
